...
{  
  ...
  ROS_INFO("move_turtle_rotate_anticlockwise ended!");  
  ros::shutdown();  
}  
int main(int argc, char** argv)  
{  
  ros::init(argc, argv, "move_turtle_rotate_anticlockwise");//初始化ROS,它允许ROS通过命令行进行名称重映射  
  ...
  ROS_INFO("move_turtle_rotate_anticlockwise start...");  
  geometry_msgs::Twist speed; // 控制信号载体 Twist message  
  while (ros::ok())  
  {  
    speed.linear.x = 0; // 设置线速度为0m/s，正为前进，负为后退  
    speed.angular.z = -0.3; // 设置角速度为0.1 rad/s，正为左转，负为右转  
